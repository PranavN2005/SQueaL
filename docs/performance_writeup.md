...
| `tabs` | 100,000 | About 91 a day. Some are reservations actually following through, some are walk-ins. |
| `tab_items` | ~750,000 | Each tab gets between 5 and 10 items, picked at random. |
...
Total runtime on a recent run was about 42 seconds. Most of that is the `tab_items` batch (about 750k rows) and the faker calls for item names. Pretty bearable for a one-time load.
